...
def query_llm(statement, client, verbose=False):
    ...
    for idx, prompt in enumerate(prompts, start=1):
        try:
            ...
            reconsider_text = completion.choices[0].message.content
            if verbose:
...
def test_client_connection(client, verbose=False):
    """
    Tests if the Azure OpenAI client is working by sending a simple test query.
    
    Args:
        client (OpenAI): The OpenAI client instance to test
        verbose (bool): If True, print detailed information
    
    Raises:
        ConnectionError: If Azure OpenAI is not reachable or the connection fails
        ValueError: If the deployment is not available
    """
    if verbose:
    ...
    try:
        ...
        if "connection" in error_msg or "refused" in error_msg or "unreachable" in error_msg:
            raise ConnectionError(
                f"Cannot connect to Azure OpenAI. Please check your internet connection and endpoint.\n"
                f"Error details: {e}"
            )
        elif "deployment" in error_msg or "not found" in error_msg:
            raise ValueError(
                f"Deployment '{DEPLOYMENT_NAME}' not found. Please verify the deployment name.\n"
                f"Error details: {e}"
            )
        else:
            raise ConnectionError(
                f"Failed to communicate with Azure OpenAI: {e}"
            )
