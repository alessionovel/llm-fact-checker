...
def query_llm(statement, client, verbose=False):
    ...
    for idx, prompt in enumerate(prompts, start=1):
        try:
            # Initial call
            system_message = "You are a rigorous Fact-Checking Analyst. You function deterministically: identical inputs must always yield identical reasoning paths and conclusions."
            messages = [
            ...
            for part in client.chat(MODEL_NAME, messages=messages, stream=True):
                initial_text += part['message']['content']
            ...
            # Stream the reconsidered response
            reconsider_text = ""
            for part in client.chat(MODEL_NAME, messages=messages, stream=True):
                reconsider_text += part['message']['content']
    ...
    try:
        ...
        if "connection" in error_msg or "refused" in error_msg or "unreachable" in error_msg:
            raise ConnectionError(
                f"Cannot connect to Ollama. Please check if Ollama is running.\n"
                f"Error details: {e}"
            )
        elif "model" in error_msg or "not found" in error_msg:
            raise ValueError(
                f"Model '{MODEL_NAME}' not found. Please verify the model is available.\n"
                f"Error details: {e}"
            )
        else:
            raise ConnectionError(
